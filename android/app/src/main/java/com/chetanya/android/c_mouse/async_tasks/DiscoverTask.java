...
public class DiscoverTask extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... aVoid) {
        ...
        DatagramPacket sendPacket = new DatagramPacket(sendPacketData,
                sendPacketData.length, broadcastAddress, SocketService.SERVER_PORT);

        while(true) {
            try {
                socket.send(sendPacket);
                Log.i(TAG, String.format("Sent packet to %s:%d",
                        broadcastAddress.getHostAddress(), SocketService.SERVER_PORT));
    ...
    protected void onPostExecute(String serverIP) {
        try{
            SocketService.setServerAddress(InetAddress.getByName(serverIP));
        } catch (UnknownHostException e) {
            Log.e(TAG, "UnknownHostException: " + e.getMessage());
            e.printStackTrace();
            System.exit(1);
